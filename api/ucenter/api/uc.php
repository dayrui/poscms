...
if (!defined('DISCUZ_ROOT')) exit('未定义');
error_reporting(0);
...
if (!is_file(DISCUZ_ROOT.'./config.inc.php')) {
    exit('未启用');
}
...
if(in_array($get['action'], array('test', 'deleteuser', 'renameuser', 'gettag', 'synlogin', 'synlogout', 'updatepw', 'updatebadwords', 'updatehosts', 'updateapps', 'updateclient', 'updatecredit', 'getcreditsettings', 'updatecreditsettings'))) {
    if(function_exists("mysql_connect")) {
        ...
        require_once DISCUZ_ROOT.'./include/db_mysqli.class.php';
        $GLOBALS['db'] = new dbstuffi;
    }
...
class uc_note {
    ...
    function synlogin($get, $post) {
        ...
        $data = $this->db->fetch_first('select uid,password from ' . $this->tablepre . 'member where username=\'' . mysql_real_escape_string($username) . '\' limit 1');
        header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
        if (isset($data['uid'])) {
            _setcookie('member_uid', $data['uid'], 86400 * 365);
            _setcookie('member_cookie', substr(md5(SITE_KEY.$data['password']), 5, 20), 86400 * 365);
        }
    }
    ...
    function synlogout($get, $post) {
        ...
        if ($uid) $this->db->query('delete from '.$this->tablepre.'member_online where uid='.$uid);
    }
...
function _setcookie($var, $value, $life = 0) {
    global $cookiedomain, $timestamp, $_SERVER;
    setcookie(SITE_PREFIX.$var, $value,
