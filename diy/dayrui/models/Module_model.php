...
class Module_model extends CI_Model {
    ...
    public function import_form($dir, $siteid) {
        ...
        foreach ($data as $id => $form) {
            ...
            $this->db->query(trim(str_replace('{tablename}', $table.'_'.$tablename, $form['sql'])));
            if (strpos($form['sql'], 'tableid') === FALSE) {
                $this->db->query('ALTER TABLE `'.$table.'_'.$tablename.'` ADD `tableid` SMALLINT(6) UNSIGNED NOT NULL COMMENT \'附表id\';');
            }
            // 创建附表
            $this->db->query('DROP TABLE IF EXISTS `'.$table.'_'.$tablename.'_data_0`');
            if ($form['sql2']) {
                $this->db->query(trim(str_replace('{tablename}', $table.'_'.$tablename.'_data_0', $form['sql2'])));
            } else {
                $sql = "
        ...
        if ($_siteid) {
            ...
            if ($form) {
                foreach ($form as $t) {
                    ...
                    $sql = trim(str_replace('{tablename}', $this->db->dbprefix($siteid.'_'.$dir.'_form_'.$t['table']), $sql));
                    $this->db->query($sql);
                    // 附表
                    ...
                    $sql = trim(str_replace('{tablename}', $this->db->dbprefix($siteid.'_'.$dir.'_form_'.$t['table']).'_data_0', $sql));
                    $this->db->query($sql);
                }
            }
        } else {
    ...
    private function _get_share_category($siteid, $dir = '') {
        ...
        if (!$this->share_category[$siteid]) {
            return array();
        }

        if (!$dir) {
            return $this->share_category[$siteid];
        }

        $category = array();
        foreach ($this->share_category[$siteid] as $i => $t) {
        ...
        if ($temp) {
            foreach ($temp as $t) {
                ...
                $field = $this->db
                    ...
                    ->where('relatedname', 'mform-'.$t['module'])
                    ->order_by('displayorder ASC, id ASC')
                    ->get('field')
                    ->result_array();
                if ($field) {
                ...
                $this->mform_cache[$t['module']][$t['table']] = $t;
            }
        }
