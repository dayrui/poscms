...
class Form_model extends CI_Model {
    ...
    public function add($data) {
        ...
        if ($this->db->insert($this->prefix, $data)) {
            ...
            if (!file_put_contents($file, '<?php'.PHP_EOL.PHP_EOL
                .'require FCPATH.\'branch/fqb/D_Form.php\';'.PHP_EOL.PHP_EOL
            ...
            if (!file_put_contents($file, '<?php'.PHP_EOL.PHP_EOL
                .'require_once FCPATH.\'branch/fqb/D_Form.php\';'.PHP_EOL.PHP_EOL
                .'class '.$name.' extends D_Form {'.PHP_EOL.PHP_EOL
                .'	public function __construct() {'.PHP_EOL
                .'		parent::__construct();'.PHP_EOL
                .'	}'.PHP_EOL.PHP_EOL
    ...
    public function del($id, $sid = SITE_ID) {
        ...
        // 删除菜单
        $this->db->where('uri', 'admin/form_'.$data['table'].'/index')->delete('admin_menu');
        $this->db->where('mark', 'site-from-'.$data['table'].'')->delete('admin_menu');
    ...
    public function cache($siteid = SITE_ID) {
        ...
        foreach ($form as $t) {
            ...
            $names[$t['table']] = $cache[$t['id']] = $t;
            // 增加到后台菜单上
            $myform = $this->db->where('uri', 'admin/form_'.$t['table'].'/index')->get('admin_menu')->row_array();
            if ($myform) {
                continue;
            }
            $leftid = 0;
            $left = $this->db->where('mark', 'content-form')->get('admin_menu')->row_array();
            if (!$left) {
                $top = $this->db->where('mark', 'content')->get('admin_menu')->row_array();
                if ($top) {
        ...
        $this->dcache->set('form-name-'.$siteid, $names);
