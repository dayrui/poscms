...
class linkage extends M_Controller {
    ...
    public function data() {
		...
		$this->template->assign(array(
			...
			'menu' => $this->get_menu_v3(array(
                fc_lang('返回') => array('admin/linkage/index', 'reply'),
				fc_lang('数据管理') => array('admin/linkage/data/key/'.$key, 'windows'),
				fc_lang('添加') => array('admin/linkage/adds/key/'.$key.'_js', 'plus'),
			)),
    ...
    public function import() {
		if ($this->input->get('admin')) {
            ...
            $id = (int)$_GET['id'];
            $lid = (int)$_GET['lid'];
            !is_file(FCPATH.'branch/Linkage/'.$id.'.php') && $this->admin_msg(fc_lang('数据文件不存在无法导入'));
            ...
            // 开始导入
            $data = require FCPATH.'branch/Linkage/'.$id.'.php';
            foreach ($data as $t) {
