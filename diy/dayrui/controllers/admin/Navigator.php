...
class Navigator extends M_Controller {
    ...
    public function __construct() {
		...
		foreach ($data as $i => $name) {
			if ($name) {
                $use[$i] = $i;
				$menu[fc_lang($name)] = array('admin/navigator/index'.(isset($_GET['type']) || $i ? '/type/'.$i : ''), 'map-marker');
				$this->menu[$i] = fc_lang($name);
			}
		}
        // 设置默认选中
    ...
    public function index() {
		...
		if (IS_POST && $this->input->post('ids')) {
			...
			if ($this->input->post('action') == 'del') {
                ...
                $this->cache(1);
                $this->system_log('删除自定义链接【'.$this->menu[$this->type].'#'.@implode(',', $ids).'】'); // 记录日志
				...
				$this->cache(1);
                $this->system_log('排序自定义链接【'.$this->menu[$this->type].'#'.@implode(',', $_ids).'】'); // 记录日志
				unset($_ids, $_data);
			}
    ...
    public function add() {
		...
		if (IS_POST) {
			...
			if (isset($data['error'])) {
				...
				$this->cache(1);
                $this->system_log('添加自定义链接【'.$this->menu[$this->type].'#'.$id.'】'); // 记录日志
    ...
    public function edit() {
		...
		if (IS_POST) {
			...
			if (isset($post['error'])) {
				...
				$this->cache(1);
                $this->system_log('修改自定义链接【'.$this->menu[$this->type].'#'.$id.'】'); // 记录日志
