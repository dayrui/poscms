...
class Category_share extends M_Controller {
    ...
    public function __construct() {
        ...
        $this->template->assign(array(
            'menu' => $this->get_menu_v3(array(
                ...
                fc_lang('自定义字段') => array('admin/field/index/rname/category-share/rid/'.SITE_ID, 'plus-square'),
    ...
    public function delete($ids) {
        ...
        $data = $this->db->where_in('id', $catid)->get($this->category_share_model->tablename)->result_array();
        if ($data) {
            // 逐一删除内容
            foreach ($data as $t) {
                if (!$t['mid'] || !$this->get_cache('module', SITE_ID, $t['mid'])) {
                    continue;
                }
                ...
                $index = $this->db->where('catid', $t['id'])->get($this->content_model->prefix.'_index')->result_array();
                if ($index) {
                    foreach ($index as $r) {
                        $this->content_model->delete_for_id((int)$r['id'], (int)$r['tableid']);
                    }
                }
            }
        }
        ...
        foreach ($catid as $id) {
            // 删除导航数据
            $this->db->where('mark', 'module-share-'.$id)->delete(SITE_ID.'_navigator');
            // 删除栏目附件
    ...
    protected function _get_tree($data) {
        ...
        foreach($data as $t) {
            ...
            // 可操作的权限判断
            $t['tid'] == 1 && $this->is_auth('admin/field/index') && $t['option'].= '<label><a class="alist onloading" href='.$this->duri->uri2url('admin/field/index/rname/share-'.SITE_ID.'/rid/'.$t['id']).'> <i class="fa fa-plus-square"></i> '.fc_lang('附加字段').'('.(int)count($category[$t['id']]['field']).')</a></label>';

            if ($this->is_auth('admin/category_share/add') && $t['tid'] != 2) {
                $t['option'].= '<label><a class="aadd onloading" href='.dr_url('category_share/add', array('id' => $t['id'])).'> <i class="fa fa-plus"></i> '.fc_lang('添加子类').'</a></label>';
            }
            if ($this->is_auth('admin/category_share/edit')) {
                $t['option'].= '<label><a class="aedit onloading" href='.dr_url('category_share/edit', array('id' => $t['id'])).'> <i class="fa fa-edit"></i> '.fc_lang('修改').'</a></label>';
            }
            if ($t['tid']==1 && !$t['child'] && $this->is_auth($t['mid'].'/admin/home/add')) {
                $t['option'].= '<label><a class="aadd onloading" href='.dr_url($t['mid'].'/home/add', array('catid' => $t['id'])).'> <i class="fa fa-pencil"></i> '.fc_lang('发布内容').'</a></label>';
            }
            if ($t['tid']==1 && !$t['child']) {
                $t['option'].= '<label><a class="ago onloading" href='.dr_url($t['mid'].'/home/index', array('catid' => $t['id'])).'> <i class="fa fa-navicon"></i> '.fc_lang('管理').'</a></label>';
            }
            // 判断是否生成静态
            if ($t['setting']['html']) {
    ...
    public function index() {
        ...
        $this->dtree->nbsp = '&nbsp;&nbsp;&nbsp;';
        $tree = array();
        $data = $this->category_share_model->repair();
        if ($data) {
    ...
    public function add() {
        ...
        if ($id){
            ...
            $data['setting']['seo']['list_title'] = '[第{page}页{join}]{catpname}{join}{SITE_NAME}';
            $data['setting']['seo']['show_title'] = '[第{page}页{join}]{title}{join}{catpname}{join}{SITE_NAME}';
            $data['setting']['seo']['extend_title'] = '{extend}{join}{title}{join}{catpname}{join}{SITE_NAME}';
        }
    ...
    public function edit() {
        ...
        $page = (int)$this->input->get('page');
        $result	= '';
        !$data && $this->admin_msg(fc_lang('栏目数据不存在'));
        ...
        if (IS_POST) {
            ...
            $this->admin_msg(fc_lang('操作成功'), $_SERVER['HTTP_REFERER'] ? $_SERVER['HTTP_REFERER'] : dr_url('category_share/edit', array('id' => $id)), 1, 1);
        }
