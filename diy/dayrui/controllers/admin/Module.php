...
class Module extends M_Controller {
    ...
    public function copy() {
		if ($this->is_auth('admin/module/config')) {
			...
			if (IS_POST) {
				...
				$this->_copy_file(TPLPATH.'pc/web/'.SITE_TEMPLATE.'/'.$dir, TPLPATH.'pc/web/'.SITE_TEMPLATE.'/'.$data['dirname']);
				$this->_copy_file(TPLPATH.'mobile/web/'.SITE_TEMPLATE.'/'.$dir, TPLPATH.'mobile/web/'.SITE_TEMPLATE.'/'.$data['dirname']);
				if ($data['name']) {
    ...
    public function install_all() {
        ...
        // 安装当前站点的数据表
        $rt  = $this->module_model->install($id, $dir, SITE_ID, $cfg, $nodb);
        if ($rt != 'ok') {
            $this->admin_msg(fc_lang('模块(%s)安装站点失败', $dir));
        }
    ...
    public function install2() {
        ...
        $data = $this->module_model->get($id);
        if (!$data) {
            $this->admin_msg(fc_lang('数据被删除或者查询不存在'));
        }
