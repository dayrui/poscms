        ...
        if (!$data) {
            ...
            if ($auth != md5(SYS_KEY)) {
                ...
                if (!$data) {
                    ...
                    if ($param == 'login') {
                    ...
                    } elseif ($param == 'send_sms_code') {
                        // 发送短信验证码
                        $post = $_REQUEST['data'];
                        if (!$post) {
                            $data = array('msg' => fc_lang('没有获取到data数组'), 'code' => 0);
                        } elseif (!$post['mobile']) {
                            $data = array('msg' => fc_lang('没有获取到手机号码'), 'code' => 0);
                        } else {
                            $code = dr_randcode();
                            $rt = $this->member_model->sendsms($post['mobile'], fc_lang('尊敬的用户，您的本次验证码是：%s', $code));
                            if ($rt['status']) {
                                $post['uid'] && $this->db->where('uid', (int)$post['uid'])->update('member', array('randcode' => $code));
                                $data = array('msg' => $rt['msg'], 'code' => 1, 'data' => $code);
                            } else {
                                $data = array('msg' => fc_lang('发送失败'), 'code' => 0);
                            }
                        }
                    } elseif ($param == 'send_sms') {
                        // 发送短信
                        $post = $_REQUEST['data'];
                        if (!$post) {
                            $data = array('msg' => fc_lang('没有获取到data数组'), 'code' => 0);
                            ...
                            $data = array('msg' => fc_lang('没有获取到短信内容'), 'code' => 0);
                        } else {
                            $rt = $this->member_model->sendsms($post['mobile'], $post['content']);
                            if ($rt) {
                        ...
                        if (!$post) {
                            ...
                            $data = array('msg' => fc_lang('没有获取到邮件地址'), 'code' => 0);
                        } elseif (!$post['title']) {
                            $data = array('msg' => fc_lang('没有获取到邮件标题'), 'code' => 0);
                        } elseif (!$post['content']) {
                            $data = array('msg' => fc_lang('没有获取到邮件内容'), 'code' => 0);
                        } else {
                            $rt = $this->member_model->sendmail($post['email'], $post['title'], $post['content']);
                            if ($rt) {
                                $data = array('msg' => fc_lang('发送成功'), 'code' => 1);
                            } else {
                                $data = array('msg' => fc_lang('发送失败'), 'code' => 0);
                            }
                        }
