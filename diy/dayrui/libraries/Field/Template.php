...
class F_Template extends A_Field {
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
        ...
        if (APP_DIR && APP_DIR != 'member' && is_dir(FCPATH.'module/'.APP_DIR)) {
            // 模块
            $files = directory_map(TPLPATH.'pc/web/'.SITE_TEMPLATE.'/'.APP_DIR.'/', 1);
        } elseif (APP_DIR == 'member' || IS_MEMBER) {
            if (defined('IS_SPACE_THEME')) {
                // 会员空间
                $files = directory_map(WEBPATH.'statics/space/'.IS_SPACE_THEME.'/', 1);
            } else {
                // 会员中心
                $files = directory_map(TPLPATH.'pc/member/'.SITE_TEMPLATE.'/common/', 1);
            }
        } else {
            // 网站
            $files = directory_map(TPLPATH.'pc/web/'.SITE_TEMPLATE.'/common/', 1);
        }
        if ($files) {
			foreach ($files as $t) {
				if ($t && strpos($t, '.html')) {
