...
class F_Ueditor extends A_Field {
				...
				<div class="form-group">
                    ...
                    <fieldset class="blue pad-10">
						...
						<div class="form-group">
							...
							<div class="form-group">
								<label class="col-md-3 control-label">'.fc_lang('Div标签转换为P标签').'：</label>
								<div class="col-md-9">
                                <div class="radio-list">
								    <label class="radio-inline"><input type="radio" value="0" name="data[setting][option][divtop]" '.($option['divtop'] == 0 ? 'checked' : '').' > '.fc_lang('开启').'</label>
								    <label class="radio-inline"><input type="radio" value="1" name="data[setting][option][divtop]" '.($option['divtop'] == 1 ? 'checked' : '').' > '.fc_lang('关闭').'</label>
								</div>
								</div>
							</div>
							<div class="form-group">
    ...
    public function insert_value($field) {
        ...
        if ($field['fieldname'] == 'content'
            ...
            } elseif (preg_match("/(src)=([\"|']?)([^ \"'>]+\.(gif|jpg|jpeg|png))\\2/i", $value, $img)) {
                // 筛选是否存在图片
                $this->ci->data[1]['thumb'] = (string)$img[3];
            }
        }
    ...
    public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
        ...
        $divtop = isset($cfg['option']['divtop']) && $cfg['option']['divtop'] ? (int)$cfg['option']['divtop'] : 0;
        ...
        if (!defined('DAYRUI_UEDITOR_LD')) {
            $str.= '
			<script type="text/javascript">var DR_DIV2P = '.($divtop ? 'false' : 'true').';</script>
