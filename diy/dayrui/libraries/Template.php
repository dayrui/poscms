    ...
    public function list_tag($_params) {
        ...
        switch ($system['action']) {
            ...
            case 'table': // 表名查询
                ...
                if (!$tableinfo) {
                    return $this->_return($system['return'], '表结构缓存不存在（排查方式：后台-系统-系统维护-数据结构，查看是否显示正常的表结构）'); // 没有表结构缓存时返回空
                }

                $table = $this->ci->db->dbprefix($system['table']); // 主表
                if (!isset($tableinfo[$table]['field'])) {
                    return $this->_return($system['return'], '表（'.$table.'）结构缓存不存在（排查方式：后台-系统-系统维护-数据结构，查看是否显示正常的表结构）');
                }
            ...
            case 'member': // 会员信息
                ...
                if (!$tableinfo) {
                    // 没有表结构缓存时返回空
                    return $this->_return($system['return'], '表结构缓存不存在（排查方式：后台-系统-系统维护-数据结构，查看是否显示正常的表结构）');
                }
