...
class Dmail {
    ...
    public function send($toemail, $subject, $message, $fname = '') {
        ...
        if(!$fp = @fsockopen($cfg['server'], $cfg['port'], $errno, $errstr, 30)) {
		    $this->runlog($cfg['server'].' - '.$cfg['auth_username'].' - '.$toemail, "连接失败 - 无法连接邮件服务器");
		    return FALSE;
	    }
	    ...
	    if($cfg['auth']) {
		    ...
		    $lastmessage = fgets($fp, 512);
	        if(substr($lastmessage, 0, 3) != 334) {
				$this->runlog($cfg['server'].' - '.$cfg['auth_username'].' - '.$toemail, "账号异常 - $lastmessage");
			    return FALSE;
		    }
		    fputs($fp, base64_encode($cfg['auth_password']) . "\r\n");
		    $lastmessage = fgets($fp, 512);
		    if(substr($lastmessage, 0, 3) != 235) {
				$this->runlog($cfg['server'].' - '.$cfg['auth_username'].' - '.$toemail, "密码异常 - $lastmessage");
			    return FALSE;
		    }
		    $email_from = $cfg['from'];
	    }
	    ...
	    if(substr($lastmessage, 0, 3) != 250) {
		    ...
		    $lastmessage = fgets($fp, 512);
		    if(substr($lastmessage, 0, 3) != 250) {
				$this->runlog($cfg['server'].' - '.$cfg['auth_username'].' - '.$toemail, "发送账号异常 - $lastmessage");
			    return FALSE;
		    }
	    }
	    ...
	    if(substr($lastmessage, 0, 3) != 354) {
				$this->runlog($cfg['server'].' - '.$cfg['auth_username'].' - '.$toemail, "数据异常 - $lastmessage");
		    return FALSE;
	    }
