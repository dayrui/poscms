...
// 搜索商城属性
function dr_mall_property($catid, $all = 0, $mdir) {

    if (!is_dir(FCPATH.'module/'.$mdir.'/')) {
        return NULL;
    }
...
function dr_space_mall_list_url($uid, $params, $name, $value, $page = FALSE) {
    ...
    if ($rule) {
        return dr_url_prefix('rewrite', $space, array(), SITE_FID).str_replace(array('{uid}', '{search}', '{page}'), array($uid, $value, '[page]'), $rule);
    } else {
        return dr_url_prefix('php', $space, array(), SITE_FID).($domain ? '' : 'uid='.$uid.'&').'action=mall&search='.$value.($page ? '&page=[page]' : '');
    }
}
