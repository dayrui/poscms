...
function dr_html_auth($is = 0) {
    ...
    $file = WEBPATH.'cache/templates/'.md5($_SERVER['HTTP_USER_AGENT']).'.auth';
    if ($is) {
        // 存储值
...
function dr_help_url($id) {
    return 'http://help.poscms.net/'.$id.'.html';
}
...
function get_module($dirname, $siteid = SITE_ID) {
    $ci = &get_instance();
    $dirname = $dirname == 'MOD_DIR' && defined('MOD_DIR') ? MOD_DIR : $dirname;
    return $ci->get_cache('module-'.$siteid.'-'.$dirname);
}
