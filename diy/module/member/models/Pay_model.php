...
class Pay_model extends CI_Model{
    ...
    public function pay_success($sn, $money, $note = '') {
        ...
        if ($data['module']) {
            if ($data['module'] == 'order') {
                ...
                $order->pay($this->db->where('id IN ('.$data['order'].')')->get(SITE_ID.'_order')->result_array(), $money, $id);
            } else {
                // 其他模块
                require_once FCPATH.'module/'.$data['module'].'/models/Order_model.php';
                $order = new Order_model();
                $order->member = $this->member;
                $order->payname = $this->db->dbprefix('member_paylog');
    ...
    public function add_for_online($pay, $money, $module = APP_DIR, $order = array()) {
        ...
        $order = $order ? (is_array($order) ? implode(',', $order) : $order) : '';
        ...
        if ($order) {
            if ($module == 'order') {
                $title = fc_lang('会员(%s)购物消费，购物订单ID：%s', $this->member['username'], $order);
                // 删已有未支付的订单
                $this->db
                     ->where('uid', $this->uid)
                     ->where('status=0 and id<>'.$id)
                     ->where('order', $order)
                     ->delete('member_paylog');
            } else {
                $sn = 'FC-'.$id.'-'.$this->uid.'-'.strtoupper($module).'-'.$order;
                $title = fc_lang('会员(%s)购物消费，购物订单ID：%s', $this->member['username'], strtoupper($module).'-'.$order);
            }
        } else {
    ...
    public function pay_for_online($id) {
        ...
        if ($data['order']) {
            if ($data['order'] == 'order') {
                $title = fc_lang('会员(%s)购物消费，购物订单ID：%s', $this->member['username'], $data['order']);
            } else {
