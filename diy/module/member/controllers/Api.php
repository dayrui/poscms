...
class Api extends M_Controller {
    ...
    public function related() {
            ...
            if (isset($data['keyword']) && $data['keyword']
                ...
                if ($data['field'] == 'id') {
                    ...
                    if (is_numeric($data['keyword'])) {
                        ...
                        $link && $this->db->where($data['field'].' IN ('.$link.')');
                    } else {
                        // 联动菜单名称查询
                        $id = (int)$this->get_cache('linkid-'.SITE_ID, $field[$data['field']]['setting']['option']['linkage']);
                        $id && $this->db->where($data['field'].' IN (select id from `'.$this->db->dbprefix('linkage_data_'.$id).'` where `name` like "%'.$data['keyword'].'%")');
                    }
                } else {
                    // 其他模糊搜索
                    $this->db->like($data['field'], urldecode($data['keyword']));
                }
            }
        }
        ...
        sort($field);
        $list = $this->db->limit($limit) ->order_by('updatetime DESC')->select('id,title,updatetime,url')->get($site.'_'.$dirname)->result_array();
    ...
    public function baidumap() {
        ...
        $this->template->assign(array(
            ...
            'width' => $this->input->get('width', true),
    ...
    public function sns_upload() {
        ...
        !$uid && exit(json_encode(array('status' => 0, 'data' => fc_lang('游客不允许上传附件'))));

        // 根据页面传入的session来获取当前登录uid，未获取到uid时提示游客无法上传
        $this->member = $this->member_model->get_member($uid); // 获取会员信息

        // 游客不允许上传，未获取到会员信息时提示游客无法上传
        !$this->member && exit(json_encode(array('status' => 0, 'data' => fc_lang('游客不允许上传附件'))));
