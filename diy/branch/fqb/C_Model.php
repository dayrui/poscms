...
class C_Model extends CI_Model {
    ...
    public function add($data, $syncatid = 0) {
        ...
        if ($data[1]['status'] >= 9) {
            ...
            // 同步共享表
            defined('IS_SHARE') && IS_SHARE && $this->update_share($data[1]);
            // 同步其他栏目
            if ($syncatid) {
    ...
    public function edit($_data, $data, $oid = 0) {
        ...
        if ($data[1]['status'] >= 9) {
            ...
            // 同步共享表
            if (defined('IS_SHARE') && IS_SHARE) {
                $this->db->where('id', intval($_data['id']))->update($this->share_prefix.'_index', array(
                    'status' => $data[1]['status']
                ));
                $this->db->where('id', intval($_data['id']))->update($this->share_prefix, array(
                    'status' => $data[1]['status']
                ));
            }
            // 更新审核表
    ...
    public function edit_extend($_data, $data) {
        ...
        if ($data[1]['status'] >= 9) {
            ...
            if (defined('IS_SHARE') && IS_SHARE) {
                $this->update_share($data[1], 1);
                // 更新索引表
                $this->db->where('id', $_data['id'])->update($this->share_prefix.'_extend_index', array(
                    'uid' => $data[1]['uid'],
                    'catid' => $data[1]['catid'],
                    'status' => $data[1]['status']
                ));
            ...
            if (defined('IS_SHARE') && IS_SHARE) {
                // 更新主表
                $this->db->where('id', (int)$_data['id'])->update($this->share_prefix.'_extend', array(
                    'status' => $data[1]['status']
                ));
                // 更新索引表
                $this->db->where('id', intval($_data['id']))->update($this->share_prefix.'_extend_index', array(
                    'status' => $data[1]['status']
                ));
            }
            // 更新审核表
    ...
    // 更新到共享表
    public function update_share($data, $extend = 0) {

        $table = $extend ? $this->share_prefix.'_extend' : $this->share_prefix;
        $field = $this->ci->get_table_field($table);
        if (!$field) {
            return;
        }

        $update = array();
        foreach ($field as $name => $t) {
            $update[$name] = $data[$name] ? $data[$name] : (strpos($t['type'], 'int') !== false ? 0 : '');
        }
        
        $update['mid'] = IS_SHARE;

        $this->db->replace($table, $update);
    }
