...
class D_Common extends CI_Controller {
    ...
    private function _init_variable() {
        ...
        define('SITE_URL', dr_http_prefix(DOMAIN_NAME.'/'));
        ...
        foreach ($sitecfg as $file) {
            ...
            if (is_file(WEBPATH.'config/site/'.$file)) {
                ...
                $this->site_info[$id]['SITE_PC'] = SITE_URL;
                $this->site_info[$id]['SITE_MURL'] = SITE_URL;
       ...
       define('SITE_ID', 1);
        ...
        $config1['SYS_NAME'] = $config1['SYS_NAME'] ? $config1['SYS_NAME'] : 'FineCMS';
        ...
        // 后台域名
        if ($config3['SYS_DOMAIN']) {
            if (IS_ADMIN && $config3['SYS_DOMAIN'] != DOMAIN_NAME) {
                // 非后台域名时,禁止访问
                exit('系统禁止访问');
            } elseif (!IS_ADMIN && $config3['SYS_DOMAIN'] == DOMAIN_NAME && !count($_GET)) {
                // 使用后台域名访问前台时(生成除外),转向
                redirect($this->site_info[SITE_ID]['SITE_URL'], '', '301');
                exit;
            }
        ...
        $config3['SYS_ATTACHMENT_URL'] = SITE_URL.$config3['SYS_UPLOAD_DIR'];
        ...
        if (is_file(WEBPATH.'config/branch.php')) {
            $this->branch = require WEBPATH.'config/branch.php';
            if ($this->branch) {
                foreach ($this->branch as $dir) {
                    // 定义全局标识
                    is_file(FCPATH.'branch/'.$dir.'/install.lock') && define('BRANCH_'.strtoupper($dir), 1);
                    // 加载函数库
                    is_file(FCPATH.'dayrui/helpers/'.$dir.'_helper.php') && include FCPATH.'dayrui/helpers/'.$dir.'_helper.php';
                    // 加载初始化init文件
                    is_file(FCPATH.'branch/'.$dir.'/init.php') && include FCPATH.'branch/'.$dir.'/init.php';
                }
            }
        }
        ...
        if (!IS_ADMIN && $MEMBER['setting']['ucenter'] && is_file(WEBPATH.'api/ucenter/config.inc.php')) {
        ...
        if ($this->member) {
            ...
            $theme = $MEMBER['setting']['guest']['theme'];
            $template = $MEMBER['setting']['guest']['template'];
        }
    ...
    public function new_field_input($field, $data = NULL, $cat = FALSE, $id = 'id', $html = '', $is_diy = 0) {
        ...
        $id = $id ? $id : 'id';
        $merge = $group = $diyfield = array();
        $myfield = $mymerge = $mygroup = $mark = '';
...
{value}';
    ...
    protected function _module_init() {
        ...
        // 设置模块模板
        $this->template->module(IS_SHARE ? SITE_TEMPLATE : ($module['template'] ? $module['template'] : 'default'));
        ...
        if (defined('SYS_UPDATE') && SYS_UPDATE) {
            ...
            foreach ($syn as $t) {
                !is_file(APPPATH.'controllers'.$t) && @copy(WEBPATH.'cache/update/module'.$t, APPPATH.'controllers'.$t);
            }
        }
    }
    ...
    public function html_thumb($p) {
        ...
        // 缓存文件
        $thumb_file = trim(SYS_THUMB_DIR, '/').'/'.md5("$id-$width-$height-$water-$size").'.'.$info['fileext'];
    ...
    public function html_thumb2($p) {
        ...
        if (is_numeric($id)) {
            ...
            // 输出图片的地址
            $display = trim(SYS_THUMB_DIR, '/').'/'.md5("index.php?c=image&m=thumb&p=$id-$width-$height-$autocut").'.'.$info['fileext'];
            // 远程图片下载到本地缓存目录
            if (isset($info['remote']) && $info['remote']) {
