...
class D_Common extends CI_Controller {
    ...
    private function _init_variable() {
        ...
        if ($siteid && isset($this->site_info[$siteid])) {
            ...
            } elseif (isset($this->site_info[$siteid]['SITE_MOBILE']) && $this->site_info[$siteid]['SITE_MOBILE'] && APP_DIR != 'weixin') {
                // 当前网站存在移动端域名时（微信模块除外）
        ...
        if (!$config3['SYS_UPLOAD_DIR']
            || trim($config3['SYS_UPLOAD_DIR'], '/') == 'member/uploadfile') {
            // 在当前网站目录
            $config3['SYS_UPLOAD_DIR'] = 'member/uploadfile';
