...
class D_Page extends M_Controller {
    ...
    public function __construct() {
		...
		if (IS_ADMIN) {
            $menu = array(
                fc_lang('单页管理') => array(APP_DIR.'/admin/page/index', 'adn'),
    ...
    protected function _get_page($id, $dir, $pid) {
        ...
        // 当前单页的数据
        $data = $page[$id];
        !$data && $this->goto_404_page(fc_lang('单网页（%s）不存在', $id));
	...
	protected function _page() {
        ...
        // 生成缓存
        defined('SYS_AUTO_CACHE') && SYS_AUTO_CACHE && file_put_contents(WEBPATH.'cache/page/'.md5(PAGE_CACHE_URL).'.html', $html, LOCK_EX);
	...
	protected function admin_delete($ids) {
        ...
        $this->system_log('删除单网页【#'.@implode(',', $ids).'】'); // 记录日志
	}
    ...
    protected function admin_index() {
		...
		if (IS_POST) {
			...
			if ($this->input->post('action') == 'order') {
				...
				$this->page_model->cache(SITE_ID);
                $this->system_log('排序单网页【'.@implode(',', $ids).'】'); // 记录日志
		...
		if ($data) {
			foreach($data as $t) {
				$t['option'] = '<a class="ago" href="'.$t['url'].'" target="_blank"> <i class="fa fa-send"></i> '.fc_lang('访问').'</a>';
		...
		if (IS_POST) {
            ...
            if (isset($data['error'])) {
                ...
                if (is_numeric($page)) {
                    ...
                    $this->page_model->cache(SITE_ID);
                    $this->system_log('添加单网页【#'.$page.'】'.$data[1]['name']); // 记录日志
		...
		if (IS_POST) {
            ...
            if (isset($post['error'])) {
                ...
                if (is_numeric($page)) {
                    ...
                    $this->attachment_handle($this->uid, $this->page_model->tablename.'-'.$page, $my);
                    $this->page_model->cache(SITE_ID);
                    $this->system_log('修改单网页【#'.$page.'】'.$post[1]['name']); // 记录日志
                    $this->admin_msg(fc_lang('操作成功，正在刷新...'), dr_url(APP_DIR.'/page/index'), 1);
                } else {
                    $error = array('msg' => $page);
                }
    ...
    public function option() {
        if ($this->is_auth('admin/page/edit') && IS_ADMIN) {
            ...
            if ($this->input->get('op') == 'show') {
                ...
                $this->system_log('修改网站【#'.SITE_ID.'】单页【'.$data['name'].'#'.$id.'】显示状态为：'.($value ? '可见' : '隐藏')); // 记录日志
                ...
                $this->system_log('修改网站【#'.SITE_ID.'】单页【'.$data['name'].'#'.$id.'】状态为：'.($value ? '关闭静态缓存' : '开启静态缓存')); // 记录日志
            }
