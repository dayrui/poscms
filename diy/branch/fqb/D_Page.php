...
class D_Page extends M_Controller {
	...
	protected function admin_delete($ids) {
		...
		$catid = '';
		foreach ($ids as $id) {
			$data = $this->db->select('childids')->where('id', $id)->get($this->page_model->tablename)->row_array();
			$catid.= ','.$data['childids'];
		}
		$catid = explode(',', $catid);
		...
		foreach ($catid as $id) {
			// 删除主表
			$this->db->where('id', $id)->delete($this->page_model->tablename);
			// 删除附件
			$this->attachment_model->delete_for_table($this->page_model->tablename.'-'.$id);
            // 删除导航数据
            $this->db->where('mark', 'page-'.$id)->delete(SITE_ID.'_navigator');
		}
    ...
    protected function admin_index() {
		...
		if (IS_POST) {
			...
			if ($this->input->post('action') == 'order') {
				...
				foreach ($ids as $id) {
					$this->db->where('id', $id)->update($this->page_model->tablename, $data[$id]);
				}
				$this->page_model->cache(SITE_ID);
