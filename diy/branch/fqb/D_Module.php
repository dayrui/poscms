...
class D_Module extends D_Common {
    ...
    protected function _show_buy() {
        ...
        (!$this->markrule || strlen($this->markrule) == 1) && exit($this->input->get('callback', TRUE).'('.json_encode(array('html' => '当前会员组无权限')).')');
        ...
        // 兼容性处理
        if (SYS_UPDATE) {
            if ($eid && !$this->db->query("SHOW TABLES LIKE '".$this->db->dbprefix.$table."'")->row_array()) {
                $this->db->query("CREATE TABLE IF NOT EXISTS `".$this->db->dbprefix.$table."` (
        ...
        if ($this->uid) {
            ...
            $eid && $data['is_buy'] == 0 && $data['is_buy'] = $this->db->where('cid='.(int)$data['cid'].' and uid='.$this->uid)->count_all_results($table);
        } else {
            $data['is_buy'] = 0;
        }
    ...
    protected function _category($id = 0, $dir = NULL, $page = 1) {
        ...
        if ($this->dir == 'share') {
            ...
            if ($cat['child'] && $cat['setting']['getchild']) {
                ...
                if ($temp) {
                    foreach ($temp as $i) {
                        if ($mod['category'][$i]['id'] != $id && $mod['category'][$i]['show'] && !$mod['category'][$i]['child']) {
                            $id = $i;
                            $cat = $mod['category'][$i];
                            break;
                        }
    ...
    protected function _category_html() {
        ...
        if (!$total) {
            if (!$category[$catid]['child'] ||
                ...
                // 生成栏目的列表分页
                if ($this->dir) {
                    $total = $category[$catid]['child'] ? $this->db->where_in('catid', @implode(',', $category[$catid]['childids']))->count_all_results(SITE_ID.'_'.$this->dir) : $this->db->where('catid', $catid)->count_all_results(SITE_ID.'_'.$this->dir);
                } else {
                    $total = 0;
                }
                if (!$total) {
    ...
    protected function _get_member_field($catid) {
        ...
        $category && $myfield = array_merge($myfield, $category);
