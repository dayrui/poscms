...
class D_Category extends M_Controller {
    ...
    public function delete($ids) {
        ...
        $data = $this->db->select('tableid,id')->where_in('catid', $catid)->get($this->content_model->prefix)->result_array();
        if ($data) {
            // 逐一删除内容
            foreach ($data as $t) {
    ...
    protected function _get_tree($data) {
        ...
        foreach($data as $t) {
            ...
            $this->is_auth(APP_DIR.'/admin/cfield/index') && $t['option'].= '<label><a class="alist onloading" href='.$this->duri->uri2url('admin/field/index/rname/'.APP_DIR.'-'.SITE_ID.'/rid/'.$t['id']).'> <i class="fa fa-plus-square"></i> '.fc_lang('附加字段').'('.(int)count($category[$t['id']]['field']).')</a></label>';
            $t['option'].= $this->get_option($t);
            $this->is_auth(APP_DIR.'/admin/category/add') && $t['option'].= '<label><a class="aadd onloading" href='.dr_url(APP_DIR.'/category/add', array('id' => $t['id'])).'> <i class="fa fa-plus"></i> '.fc_lang('添加子类').'</a></label>';
            $this->is_auth(APP_DIR.'/admin/category/edit') && $t['option'].= '<label><a class="aedit onloading" href='.dr_url(APP_DIR.'/category/edit', array('id' => $t['id'])).'> <i class="fa fa-edit"></i> '.fc_lang('修改').'</a></label>';
            !$t['setting']['linkurl'] && !$t['child'] && $this->is_auth(APP_DIR.'/admin/home/add') && $t['option'].= '<label><a class="aadd onloading" href='.dr_url(APP_DIR.'/home/add', array('catid' => $t['id'])).'> <i class="fa fa-pencil"></i> '.fc_lang('发布').'</a></label>';
            !$t['setting']['linkurl'] && !$t['child'] && $t['option'].= '<label><a class="ago onloading" href='.dr_url(APP_DIR.'/home/index', array('catid' => $t['id'])).'> <i class="fa fa-navicon"></i> '.fc_lang('管理').'</a></label>';
            // 判断是否生成静态
            $t['html'] = $category[$t['id']]['setting']['html'] ? '<label><a class="badge badge-success" href='.dr_url('module/html', array('id' => APP_DIR, 'sid' => SITE_ID)).'> '.fc_lang('是').' </a>' : '<a class="badge badge-warning" href='.dr_url('module/html', array('id' => APP_DIR, 'sid' => SITE_ID)).'> '.fc_lang('否').' </a></label>';
            // 外部地址不显示
            ...
            if ($catid) {
                foreach ($catid as $id) {
                    ...
                    if ($setting) {
                        ...
                        $this->db->where('id', $id)->update($this->category_model->tablename, array(
    ...
    public function rule() {
        ...
        if (IS_POST) {
            ...
            $this->db->where('id', $catid)->update($this->category_model->tablename, array(
