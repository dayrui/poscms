...
class D_File extends M_Controller {
	...
	public function index() {
        ...
        $this->template->assign(array(
			...
			'list' => $list,
			'parent' => dirname($dir),
            'upload' => 'dr_upload_files2(\''.dr_url('api/upload', array('path' => urlencode(dr_authcode($path, 'ENCODE')))).'\')',
		));
	...
	public function edit() {
		...
		$this->template->assign(array(
			...
			'body' => $this->_code($this->path.$file),
    ...
    private function _code($file) {

	    $code = file_get_contents($file);
        if (in_array(strtolower(strrchr($file, '.')), array('.js'))) {
            return $code;
        }
