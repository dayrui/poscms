...
class D_Admin_Home extends M_Controller {
    ...
    public function _get_field($catid = 0) {
        ...
        $category && $field = array_merge($field, $category);
        // 筛选出右边显示的字段
        ...
        if (IS_POST && !$this->input->post('search')) {
            ...
            switch ($action) {
                ...
                case 'flag':
                    ...
                    break;
                case 'html':
                    $url = ADMIN_URL.'index.php?'.(defined('IS_SHARE') && IS_SHARE ? '' : 's='.APP_DIR.'&').'c=show&m=html&page=1&type=html&value='.implode(',', $ids).'&total='.count($ids);
        ...
        if (IS_POST) {
            ...
            if (isset($data['error'])) {
                ...
                if (($id = $this->content_model->add($data, $syncatid)) != FALSE) {
                    ...
                    // 是否创建静态页面链接
                    $create = $category['setting']['html'] && $data[1]['status'] == 9 ? dr_module_create_show_file($id, 1) : '';
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            if (isset($post['error'])) {
                ...
                $this->admin_msg(
                    fc_lang('操作成功，正在刷新...') .
                    ($category['setting']['html'] && $post[1]['status'] == 9 ? dr_module_create_show_file($lid).dr_module_create_list_file($catid) : ''),
                    $this->_get_back_url(APP_DIR.'/home/index'),
                    1,
                    1
                );
